...
class BitFlipDataset(torch.utils.data.Dataset):
...
def evaluate_interactive(model, dataset_path="dataset_inversao_bits.json", threshold=0.5):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    dataset = BitFlipDataset(dataset_path)
    ...
    while True:
        ...
        if user_input.lower() == 'exit':
            ...
            try:
                ...
                print("\nSaída prevista (bits invertidos):")
                print(pred[0])
            ...
            try:
                ...
                print(x[0, :-1].cpu().numpy())
                print("\nSaída esperada (bits invertidos):")
                print(y_true[0].cpu().numpy())
                print("\nSaída prevista:")
                print(y_pred[0].cpu().numpy())
