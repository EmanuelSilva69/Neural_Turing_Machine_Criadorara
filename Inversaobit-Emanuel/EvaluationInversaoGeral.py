import os
import json
import random
import torch
import numpy as np
from sklearn.metrics import f1_score, accuracy_score, precision_score, recall_score
from NTM import NTM

# Dataset para inversão de bits
class BitFlipDatasetFromJSON(torch.utils.data.Dataset):
    def __init__(self, path):
        self.samples = []
        with open(path, "r") as f:
            for line in f:
                item = json.loads(line)
                x = torch.tensor(item["input"], dtype=torch.float32)
                y = torch.tensor(item["target"], dtype=torch.float32)
                self.samples.append((x, y))

    def __len__(self):
        return len(self.samples)

    def __getitem__(self, idx):
        return self.samples[idx]

#  Função para gerar dados fora da faixa de treino (generalização)
def gerar_dataset_inversao_para_teste(path="dataset_inversao_bits_teste.json", num_samples=1000, min_len=21, max_len=40, vector_size=8):
    random.seed(42)
    torch.manual_seed(42)

    data = []
    for _ in range(num_samples):
        seq_len = random.randint(min_len, max_len)
        seq = torch.randint(0, 2, (seq_len, vector_size)).float()
        eos = torch.zeros(1, vector_size)
        eos[0, -1] = 1.0
        input_seq = torch.cat([seq, eos], dim=0)
        target_seq = 1.0 - seq  # inversão de bits

        data.append({"input": input_seq.tolist(), "target": target_seq.tolist()})

    with open(path, "w") as f:
        for item in data:
            json.dump(item, f)
            f.write("\n")

    print(f"Dataset de teste para generalização salvo em '{path}' com {num_samples} exemplos.")

# Avaliação com métricas
def evaluate_metrics(model, dataset_path="dataset_inversao_bits.json", num_samples=100, threshold=0.5,
                     output_path="avaliacao_inversao_resultados.txt", examples_path="avaliacao_inversao_exemplos.txt"):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.to(device)
    model.eval()

    dataset = BitFlipDatasetFromJSON(dataset_path)
    indices = random.sample(range(len(dataset)), num_samples)

    y_true_all = []
    y_pred_all = []

    with open(examples_path, "w", encoding="utf-8") as f_ex:
        for idx in indices:
            x, y_true = dataset[idx]
            x = x.unsqueeze(0).to(device)
            y_true = y_true.unsqueeze(0).to(device)

            with torch.no_grad():
                out = model(x)
                if out.size(1) > y_true.size(1):
                    out = out[:, :-1, :]
                y_pred = (out > threshold).float()

            y_true_all.append(y_true.cpu().numpy().reshape(-1))
            y_pred_all.append(y_pred.cpu().numpy().reshape(-1))

            f_ex.write(f"Exemplo {idx}\n")
            f_ex.write("Entrada (sem EOS):\n")
            f_ex.write(str(x[0, :-1].cpu().numpy()) + "\n")
            f_ex.write("Saída esperada (bits invertidos):\n")
            f_ex.write(str(y_true[0].cpu().numpy()) + "\n")
            f_ex.write("Saída prevista:\n")
            f_ex.write(str(y_pred[0].cpu().numpy()) + "\n\n")

    y_true_all = np.concatenate(y_true_all)
    y_pred_all = np.concatenate(y_pred_all)

    f1 = f1_score(y_true_all, y_pred_all)
    acc = accuracy_score(y_true_all, y_pred_all)
    prec = precision_score(y_true_all, y_pred_all)
    rec = recall_score(y_true_all, y_pred_all)

    results = (
        f"Avaliação em {num_samples} exemplos aleatórios:\n"
        f"F1 Score: {f1:.4f}\n"
        f"Acurácia (bit a bit): {acc:.4f}\n"
        f"Precisão: {prec:.4f}\n"
        f"Recall: {rec:.4f}\n"
    )

    print("\n" + results)

    with open(output_path, "w", encoding="utf-8") as f:
        f.write(results)

# Execução principal
if __name__ == "__main__":
    # Gerar dados de teste fora da faixa de treino (21 a 40)
    gerar_dataset_inversao_para_teste(
        path="dataset_inversao_bits_teste.json",
        num_samples=3000,
        min_len=1,
        max_len=40
    )

    # Carregar modelo treinado
    model = NTM(input_size=8, output_size=8)
    model.load_state_dict(torch.load("checkpoint/best_model.pth", map_location=torch.device("cuda" if torch.cuda.is_available() else "cpu")))

    # Avaliar generalização
    evaluate_metrics(
        model=model,
        dataset_path="dataset_inversao_bits_teste.json",
        num_samples=600,
        threshold=0.5,
        output_path="avaliacao_inversao_resultados_geral.txt",
        examples_path="avaliacao_inversao_exemplos_geral.txt"
    )
